...
const Season = () => {
    ...
    return (
        <div className="m-8 flex flex-col gap-2">
            ...
            <h1 className="text-4xl text-fuchsia-300 drop-shadow-fog mb-2">{seasonProper} Birthdays</h1>
            {Array.from(SEASON_BIRTHDAYS).map((birthday: any, key) => {
                ...
                return (
                    <div className="flex gap-1 align-middle items-center" key={key}>
                        ...
                        <p className="text-2xl text-lime-500 drop-shadow-fog w-[40px]">{birthday.day}</p>
                        <Image className="w-14 h-14 drop-shadow-fog" src={villagerImage} alt={""} width={30} height={30} />
                        {itemImage && (
                            <Image className={`w-14 h-14 drop-shadow-fog`} src={itemImage} alt={""} width={50} height={50} />
                        )}
                    </div>
                )
            })}
